...
def LN_model_summary(trace, stim, nbins=30, n_testset=2000):
    ...
    nbins = np.ceil(nbins)
    nbins = int(nbins)
    n_testset = int(n_testset)
    print('nbins = ', nbins)
    print('n_testset = ', n_testset)

    n_cells = trace.shape[0]
    n_subplots = 7

    # model_r definition
    #model_r = model_r_init(["L","LN"], dataset=["training","test1","test2"], n_cell=n_cells)
    model_r = model_r_init(["LN"], dataset=["training","test1","test2"], n_cell=n_cells)
    ...
    for i in range(n_cells):
        ...
        #model_r[i]['test1']["L"]     = pearsonr(resp[-n_testset:], pred_heldout)[0]
        ...
        model_r[i]['test1']["LN"]     = pearsonr(resp[-n_testset:], LN_output_heldout_data)[0]
        ...
        print('Cell %d: w/ training data (%.3f), w/ test data (%.3f)'
              %  (i+1, model_r[i]['training']["LN"], model_r[i]['test1']["LN"]))
    ...
    model_r_bar_plot(model_r, dataset=["training","test1","test2"], models=["LN"])
    #
